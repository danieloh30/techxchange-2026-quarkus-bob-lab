...
Lab materials are split into **docs** (instructions) and **exercises** (completed Quarkus solutions):

```text
docs/                         # All lab instructions + images
├── LAB_GUIDE.md              # Full 90-minute instructor/participant guide
├── 00-intro/ … 07-a2a/       # Per-exercise START_HERE + Bob materials
└── images/

exercises/                    # Completed Quarkus projects per exercise
...
Solutions are adapted from the [Quarkus LangChain4j workshop](https://github.com/quarkusio/quarkus-workshop-langchain4j). Follow `docs/*/START_HERE.md`, then run the matching exercise project.
...
1. Open `exercises/04-ibm-bob/solution` in your IDE with Bob enabled (prompts live under `docs/04-ibm-bob/`).
...
**Instructor note:** If Bob is unavailable in the room, use the printed prompt/response cards in `docs/04-ibm-bob/FALLBACK.md` and continue — do not block the remaining exercises.
...
Client config (already in the solution `application.properties`):
...
### 6.3 Observability (see also `exercises/06-hitl-observability/observability-reference`)
...
| Bob unavailable | Use `docs/04-ibm-bob/FALLBACK.md` |
