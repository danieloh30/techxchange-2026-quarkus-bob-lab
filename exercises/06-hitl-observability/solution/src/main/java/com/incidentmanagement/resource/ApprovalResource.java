 ...
 * Provides endpoints for humans to view and approve/reject escalation proposals.
 */
@Path("/api/approvals")
...
public class ApprovalResource {
    ...
    /**
     * Make a decision on a proposal with explicit KEEP_AT_TEAM or ESCALATE_INCIDENT action.
     * Simplified approach where the UI directly specifies the desired outcome.
     *
     * @param proposalId The proposal ID
     * @param request Request body containing decision (KEEP_AT_TEAM or ESCALATE_INCIDENT), reason, and approvedBy
     */
    @POST
    ...
    public Response decideProposal(
        ...
        try {
            String decision = request.get("decision"); // KEEP_AT_TEAM or ESCALATE_INCIDENT
            ...
            if (decision == null || (!decision.equals("KEEP_AT_TEAM") && !decision.equals("ESCALATE_INCIDENT"))) {
                return Response.status(Response.Status.BAD_REQUEST)
                        .entity(Map.of("error", "Decision must be either KEEP_AT_TEAM or ESCALATE_INCIDENT"))
                        .build();
            }
