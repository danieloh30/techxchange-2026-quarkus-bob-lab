...
INSERT INTO incident_info (id, system_name, service, priority, description, status) VALUES
    (nextval('incident_info_id_seq'), 'payment-gateway', 'checkout-api', 'P2', 'Intermittent 503 errors during peak hours', 'RESOLVED'),
    (nextval('incident_info_id_seq'), 'auth-service', 'user-login', 'P1', 'Complete authentication failure', 'ESCALATED'),
    (nextval('incident_info_id_seq'), 'inventory-db', 'stock-sync', 'P3', 'Stale inventory data, sync lag 15 min', 'RESOLVED'),
    (nextval('incident_info_id_seq'), 'cdn-edge', 'static-assets', 'P4', 'Slow asset loading in EU region', 'RESOLVED'),
    ...
    (nextval('incident_info_id_seq'), 'monitoring', 'alerting-api', 'P2', 'Alert fatigue — 40% false positive rate', 'ESCALATED'),
